...
const UserTickets = () => {
  ...
  const [tickets, setTickets] = useState({
    ...
    past: [
      {
        id: "EVT-002-USER",
        eventId: "rhumba-star-2024",
        eventName: "Rhumba Star",
        date: "2025-2-20",
        time: "20:00",
  ...
  const handleViewDetails = (eventId, isUserTicket = false, ticket = null) => {
    ...
    const eventDetails =
      tickets.upcoming.find((t) => t.eventId === eventId) ||
      tickets.past.find((t) => t.eventId === eventId);
    if (eventDetails) {
      navigate(`/event/${eventId}/details`, {
        state: {
          event: eventDetails,
          fromDashboard: isUserTicket,
          userTicket: ticket,
        },
      });
    } else {
          ...
          tickets[activeTab].map((ticket) => (
            <div key={ticket.id} className={styles.ticketCard}>
              <div className={styles.ticketInfo}>
                ...
                <p>
                  <FaCalendarAlt /> {new Date(ticket.date).toLocaleDateString()}{" "}
                  at {ticket.time}
                </p>
                <p>
                  <FaMapMarkerAlt /> {ticket.venue}
                </p>
                <p>
                  <FaTicketAlt /> {ticket.ticketType} Ticket
                </p>
              ...
              <div className={styles.ticketActions}>
                ...
                <button
                  onClick={() =>
                    handleViewDetails(ticket.eventId, true, ticket)
                  }
